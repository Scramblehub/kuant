...
### 2. One function for both directions

Put-call symmetry is a mathematical fact, not an implementation choice.
Encoding it as one kernel (rather than `bsputgamma` + `bscallgamma`
aliases) makes the fact enforceable in code and un-losable in refactors.

### 3. Composition on `normpdf`, uses `d1` only

Never needs `d2` even though `prepare_bs` computes it. The extra
subtraction per element is cheaper than an if-branch and keeps the setup
helper simple. Zero-cost tradeoff.
...
- `test_gamma_matches_finite_difference_of_delta` —
  `bsgamma ≈ (bsputdelta(S+h) - bsputdelta(S-h)) / (2h)`
- `test_gamma_matches_second_derivative_of_price` —
  `bsgamma ≈ (bsput(S+h) - 2·bsput(S) + bsput(S-h)) / h²`

Both cross-checks bump through delta and price kernels, so drift anywhere
in the chain (bsgamma, bsputdelta, bsput, normcdf, normpdf) surfaces here
first.
